---
name: a11y-accessibility-inspect
...
The shared rules — severity, the no-proxy boundary, high-risk patterns, conformance, grounding — are in [`../a11y-shared/methodology.md`](../a11y-shared/methodology.md). Read it when a call needs judgment. The rules that always apply are below.
...
```bash
opencode: agregar el MCP de browser en opencode.json (ej. playwright-mcp)
...
rowser_navigate (o la herramienta de navegación del MCP conectado) needs a URL, so resolve first:
- A URL: use it.
