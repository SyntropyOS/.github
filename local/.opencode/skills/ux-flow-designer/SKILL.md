...
```
CHECK .agents/skills/ui-ux-pro-max/ exists (proyecto) o ~/.agents/skills/ui-ux-pro-max/ (global)
  IF missing → informar al usuario de la skill faltante (opencode no la tiene instalada)

CHECK docs/product/prd.md exists
  IF missing → CHECK .agents/skills/product-manager-toolkit/ o ~/.agents/skills/product-manager-toolkit/
    IF missing → aceptar input directo del usuario como descripción de features
...
- If browser MCP (Playwright) is available: use `browser_navigate` to open the file
...
1. **First**: Inform the requirements before attempting anything:
   ...
   - MCP agregado a opencode: configurar el servidor SSE en `opencode.json` (`mcp: { "figma-dev-mode-mcp-server": { "type": "sse", "url": "http://127.0.0.1:3845/sse" } }`)
