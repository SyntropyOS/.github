...
**STATUS LEGIBILITY NOTE (vantadb-lead 2026-08-05, actualizado DOD-ENFORCE-03 D14):** This file is the **specification**
for the enforcement engine. `campaign_enforce_state` (in
`campaign-server.mjs`) implements SIX of these checks in runtime:
  ...
  4. EditGuard `max_edit_lines` (ACT 100 — D14, config-driven vía `C0_CHECK_CONFIG` espejo de `STATE_TOOLS`),
  5. Bash `allowed_commands` allow-listing (config-driven — hoy VERIFY; cualquier estado con `allowed_commands` en config lo enforcea),
   6. File-scope `max_files_per_state` (ACT 5, re-edits exentos, reset al cambiar de estado — D14, `Map` en memoria keyed por sessionId + fallback `toolArgs.filesEdited` + persistencia best-effort a `enforcement/sessions/file-scope-<key>.json` para sobrevivir reinicios),
   7. Environment blocking `blocked_env` (`checkBlockedEnv` — API_KEY/TOKEN/SECRET/PASSWORD/REGISTRY_TOKEN/AUTH),
plus the Bash operation classifier (tool-based blocking when `allowed_commands` is NOT set).
The checks below labeled **[SPEC]** (read-dedup, context-budget) are NOT yet wired into `campaign_enforce_state`.
...
## 1. [RUNTIME — D14] Edit Guard — `max_edit_lines`
...
## 2. [RUNTIME] Bash Command Allow-Listing — `allowed_commands`
...
## 3. [RUNTIME] Bash Operation Classifier — tool-based blocking
...
## 4. [RUNTIME — D14] File Scope — `max_files_per_state`
